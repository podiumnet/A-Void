...
Enemy.prototype = {
  ...
  collision: function (obj) {
    if (obj instanceof Player) levelSwitch(1);
  },
  update: function () {
    if (this.dissapear) {
      if (ticks % 200 === 0) {
    ...
    if(this.move && (player.x > 10 || player.y > 10)) {
      if (this.move === 1) {
        if (ticks % 30 === 0) {
        ...
        this.y += player.y > this.y ? this.speed : -this.speed;
      }
    }
  },
  trap: true,
...
Enemy.genAmount = function () {
  return (canvas.width * canvas.height / 200000) + difficulty / 2;
};
inserts.push(Enemy);
